...
<!-- 	                            <div>{{animated_online_users_count | prettydecimal(0)}} Conectados <i class="fa fa-circle fa-1x"></i></div> -->
	                        </div>
	                    </div>
	                </div>
	            ...
	            <div class="panel panel-default">
	                <div class="panel-heading">
	                    <div class="row">
	                        ...
	                        <div class="col-xs-9 text-right">
	                            <div>{{animated_webpages_count | prettydecimal(0)}} Web Pages <i class="fa fa-users fa-1x"></i></div>
	                        </div>
	                    </div>
	                </div>
	                <a href="#" v-on:click="show_webpages = !show_webpages">
						<div class="panel-footer">
							<span class="pull-left">
								<span v-if="!show_webpages">Show webpages</span>
								<span v-else>Hide webpages</span>
							</span>
							<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
							<div class="clearfix"></div>
						</div>
...
<!-- 		                        <div><span class="pull-right"> Ir</a></span></div> -->
	                        </div>
	                    </div>
	                </div>
	            </div>
        ...
        <div class="row" v-if="show_webpages">
	        <div v-for="webpage in webpages" class="col-lg-6 col-md-6">
	            <div class="panel panel-white">
	                <div class="panel-heading">
	                    <div class="row">
	                        <div class="col-xs-3">
<!-- 	                            <img :src="user | user_image_url" class="circular--square" with="48px" height="48px"/> -->
	                        ...
	                        <div class="col-xs-9 text-right">
		                        <a :href="webpage | webpage.slug"><div> {{webpage.title}}</div></a>
		                        <div v-for="section in webpage.sections" class="col-lg-6 col-md-6">
			                		<div class="col-xs-3 text-right">        
				                		<!-- <a :href="section | section.slug"><div> {{section.title }}</div></a> -->
		...
		new Vue({
		  ...
		  data () {
		    return {
			  ...
			  users_count: 0,
			  users: [],
			  tweened_page_items_count: 0,
			  tweened_webpages_count: 0,
...
// 			  var val = parseFloat(object.liters) / parseFloat(object.liters) * 100;
		  ...
		  computed: {
		    animated_page_items_count: function() {
		      return this.tweened_page_items_count.toFixed(2);
		    },
			animated_webpages_count: function() {
		      return this.tweened_webpages_count.toFixed(2);
		    },
		  ...
		  watch: {
		    page_items_count: function(newValue) {
		      gsap.to(this.$data, { duration: 1, tweened_page_items_count: newValue });
		    },
		    webpages_count: function(newValue) {
		      gsap.to(this.$data, { duration: 1, tweened_webpages_count: newValue });
		    },
		    users_count: function(newValue) {
		      gsap.to(this.$data, { duration: 1, tweened_users_count: newValue });
		    },
		  },
		  mounted () {			  
			  ...
			  dashboard_poll: function () {
				  //use axios for the request, and mount data into global variables for the app
				   axios
				      ...
				      .then(response => (
					      this.page_items_count = response.data.response.page_items_count,
					      this.webpages = response.data.response.webpages,
