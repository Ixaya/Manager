...
class Websockets extends CI_Controller
{
	...
	public function serve()
	{
		...
		$this->config->load('websocket', TRUE, TRUE);

		// Setup logging
		$logHandler = new StreamHandler(getStdout());
		$logHandler->setFormatter(new ConsoleFormatter());
		$logHandler->setLevel(\Monolog\Level::Debug);
		$this->logger = new Logger('websocket-server');
		$this->logger->pushHandler($logHandler);

		// Create HTTP server with custom 
		$config = $this->config->item('websocket');
		$maxConnectionsPerIp = $config['max_connections_per_ip'] ?? 100;
		...
		$clientHandler = new WebsocketsClientHandler($this->gateways, $this->logger);
		...
		$this->logger->info("Received signal {$signal}, stopping server");
...
class WebsocketsClientHandler implements WebsocketClientHandler {
	...
	public function __construct(array &$gateways, LoggerInterface $logger)
	{
