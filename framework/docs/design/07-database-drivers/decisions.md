...
- **2026-08-05: `pdo/*` variants are NOT engine-matrix rows.** A routine
  ...
  Recorded in `framework/docs/development/framework-workflow.md`'s "Cross-engine
  verification" and `framework/docs/development/spec-campaigns.md`'s live-testing
  list.
...
- **2026-08-05 (closing review): the ~2x was a togglable default, not a
  ...
  (`framework/docs/development/docker-decisions.md`,
...
Absorbed from `framework/docs/workspace/22-timestamp-timezone-mapping/` at
...
- **2026-08-10: `MgrFieldType::Timestamp` repointed from a bare `TIMESTAMP`
  ...
  regardless; see `framework/docs/development/database.md` for that naming collision
...
- **2026-08-10: an `unsigned: true` field on PostgreSQL or SQL Server is
  ...
  untouched. Full mechanism in `framework/docs/development/database.md`; the
...
- **2026-08-10: the write-side counterpart — whether
  ...
  Full write-up: `framework/docs/workspace/00-proposals/timestamp-write-format-atom/spec.md`.
...
mechanics in `framework/docs/development/database.md`; this section is the rationale.
...
- **2026-08-15: `add_column()` cannot add an AUTO_INCREMENT column to an
  ...
  `framework/docs/workspace/00-proposals/add-column-auto-increment-mysql/spec.md`.
