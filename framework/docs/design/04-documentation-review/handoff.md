...
`framework/docs/development/skill-authoring.md`. That document is the durable output of
...
| Document | State |
|---|---|
| `framework/docs/development/skill-authoring.md` | New. Writing and reviewing skills, including the review checklist for validating one written by a weaker model |
| `framework/docs/development/framework-workflow.md` | New. The development loop, the quality gates, and where a change goes — including the three-file shape of a framework library |
| `sample/docs/development/upgrading.md` | New, shipped. Six-step upgrade procedure, pointed at from `README.md` and `SETUP.md` |
| `framework/docs/architecture/framework-wiring.md` | Renamed from `mx-boot-wiring.md` and grown; now carries class resolution (`MGR_*` → package alias → `MY_`/`APP_`) as a section of its own |
...
Rule changes to `sample/docs/documentation.md` and `framework/docs/documentation.md` are
...
`framework/docs/development/skill-authoring.md` — was closed by campaign
`16-documentation-improvements` (archived
`framework/docs/workspace/archive/16-documentation-improvements.tar.xz`): all ten
