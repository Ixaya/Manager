...
# that sets DEST_LOG paths and calls compress_logs() for each site that
# user serves. See samples/logs_compress.sh.
...
CUTOFF_DATE=$(date -d "1 month ago" "+%Y/%m" 2>/dev/null || date -v-1m "+%Y/%m" 2>/dev/null)
...
compress_logs() {
    ...
    echo "Compressing folders older than: $CUTOFF_DATE"
    echo "========================================"
