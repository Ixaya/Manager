...
### `MGR_Model`'s write methods: `replace()` narrowed to three engines, and a returned id is typed by the driver
...
**`replace()` was split into two methods with different names, then
partially reinstated.** It used to pick a SQL mechanism per driver, which
meant one call had genuinely different semantics depending on where it ran
— a partial payload wiped the omitted columns on MySQL and preserved them
on Postgres. It was replaced with:
...
Neither covers matching against whatever unique constraint a row happens to
conflict on, without naming it up front — the one thing native `REPLACE
INTO`/`INSERT OR REPLACE` do. So on MySQL/MariaDB/SQLite, `replace(array
$data): int|string|bool` is back, unchanged from before: it deletes any row
conflicting on a primary or unique key, then inserts `$data`, so an omitted
column resets to its default rather than keeping the replaced row's value.
It still throws `RuntimeException` on Postgres and SQL Server — those two
engines only ever get `replace_pk()`/`upsert_atomic()`.

Pick by meaning: `replace()`/`replace_pk()` discard the old row,
`upsert_atomic()` merges into it. Pick by availability: `replace()` only
exists on MySQL/MariaDB/SQLite, `upsert_atomic()` only on Postgres.
