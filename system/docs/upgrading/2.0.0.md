...
update them to the new parents/patterns, keeping project logic.
`sample/application/core/APP_Site_Controller.php` is a real, minimal worked
example of the pattern; `Admin_Controller`/`Private_Controller` have none —
see the `mgr-web-controllers` skill for the theming contract they must
follow. One 2.0 requirement is easy to miss:
