<template>
  <div>
    ...
    <sub-header title="Usuarios" description="" :button="true" button_text="Crear usuario" button_icon="bi-person-fill-add" button_class="btn-success" button_link="default.user-add"></sub-header>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-danger" @click="onDeleteSubmit(userId, userName, 1)" data-bs-dismiss="modal">Eliminar</button>
        </div>
      </div>
    </div>
  </div>
...
import { useUsers } from '@/common/composables/user/useUsers'
import { useDeleteUser } from '@/common/composables/user/useDeleteUser'
...
const { data, params, isLoading, refetch } = useUsers()
const deleteModal = ref(null)
const DataTableOptions = ref({
  columns: [{ title: 'ID' }, { title: 'Nombre Completo' }, { title: 'Email' }, { title: 'Dirección IP' }, { title: 'Creado en' }, { title: 'Última actividad' }],
  data: formattedData.value
})
const header = [{ title: 'ID' }, { title: 'Nombre Completo' }, { title: 'Email' }, { title: 'Dirección IP' }, { title: 'Creado en' }, { title: 'Última actividad' }, { title: 'Acción' }]
